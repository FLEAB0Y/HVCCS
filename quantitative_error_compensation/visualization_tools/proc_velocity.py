...
save_dir = os.path.join(project_root, 'res', 'feature_visualization_velocity')
...
for filename in os.listdir(contra_features_dir):
    if filename.endswith('.txt'):
        ...
        # 解析选中行的数据，只读取第53-151列（0-based索引52:151），每行是一帧，每列是一种特征
        data = []
        ...
        # 计算33个特征的帧间导数（每个特征有3列xyz），并绘制速度幅值（frame-to-frame difference magnitude）
        num_landmarks = 33
        if num_frames < 2:
            print(f"{filename} 帧数不足以计算导数，跳过。")
            plt.close()
            continue
        x_deriv = np.arange(num_frames - 1)  # 导数对应的帧索引（帧间差）
        for i in range(num_landmarks):
            ...
            z_col = i * 3 + 2
            # 检查列索引是否超出范围
            if max(x_col, y_col, z_col) >= num_features:
                print(f"{filename} 的特征列不足以包含 landmark {i+1}，停止后续landmark处理。")
                break
            # 计算相邻帧差值（导数近似）：dx, dy, dz，然后计算速度幅值
        ...
        plt.title(f'{filename} landmark derivative (frame-to-frame speed) visualization (33 landmarks)')
